...
const heroLines = ["Precision, illuminated.", "Intelligence, made visceral."];
const heroStats = [
  { value: "12ms", label: "Signal response layer" },
  { value: "06", label: "Surfaces unified in one canvas" },
  { value: "99.96%", label: "Operational confidence" }
];
...
export function Hero() {
  ...
  return (
    <section
      ref={root}
      className="relative flex min-h-screen items-end overflow-hidden pt-28 sm:pt-32"
    >
      ...
      <div className="section-shell section-space relative grid min-h-[calc(100vh-7rem)] items-center gap-14 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="max-w-3xl">
          <span className="section-kicker hero-kicker">Launch Collection 01</span>

          <div className="mt-8 space-y-2">
            {heroLines.map((line) => (
              <span key={line} className="line-mask">
                <span className="hero-line-inner font-display block text-5xl font-semibold leading-[0.92] tracking-[-0.065em] text-white sm:text-6xl md:text-7xl xl:text-[6.6rem]">
                  {line}
                </span>
              </span>
            ))}
          ...
          <p className="hero-copy mt-8 max-w-2xl text-base leading-8 text-white/70 sm:text-lg">
            Astra One is the premium command surface for ambitious teams that want every
            decision to feel instant, cinematic, and unmistakably in control.
          </p>

          <div className="mt-10 flex flex-col gap-4 sm:flex-row sm:items-center">
            <div className="hero-action">
              <ActionButton href="#cta">Request Private Preview</ActionButton>
            </div>
            <div className="hero-action">
              <ActionButton href="#story" variant="secondary">
          ...
          <div className="mt-12 grid gap-4 sm:grid-cols-3">
            {heroStats.map((stat) => (
              <div
                key={stat.label}
                className="hero-stat rounded-[1.5rem] border border-white/[0.08] bg-white/[0.03] p-4 backdrop-blur-xl"
              >
        ...
        <div className="hero-visual-stack relative mx-auto w-full max-w-[34rem]">
          <div className="hero-visual glass-panel relative overflow-hidden rounded-[2rem] border-white/[0.12] p-5">
            ...
            <div className="relative rounded-[1.5rem] border border-white/[0.12] bg-[linear-gradient(180deg,rgba(255,255,255,0.08),rgba(255,255,255,0.01))] p-5">
              <div className="flex items-center justify-between text-[0.7rem] uppercase tracking-[0.24em] text-white/[0.42]">
              ...
              <div className="mt-6 rounded-[1.35rem] border border-white/[0.08] bg-background-soft/[0.80] p-6">
                <div className="grid gap-4 sm:grid-cols-[0.9fr_1.1fr]">
                  <div className="rounded-[1.15rem] border border-white/[0.08] bg-white/[0.03] p-4">
                    ...
                    <p className="mt-5 font-display text-3xl font-semibold tracking-[-0.05em] text-white">
                      AETHER
                    </p>
                    <p className="mt-3 text-sm leading-6 text-white/[0.56]">
                      Taste-driven orchestration with executive-grade clarity and adaptive
                      context routing.
                    </p>
                  </div>
                  ...
                  <div className="relative rounded-[1.15rem] border border-white/[0.08] bg-[radial-gradient(circle_at_top,rgba(152,181,255,0.22),transparent_55%),rgba(255,255,255,0.03)] p-4">
                    ...
                    <div className="relative flex h-52 items-end justify-between">
                      {[58, 74, 92, 82, 96, 71].map((height) => (
                        <span
                          key={height}
                          className="w-[13%] rounded-full bg-[linear-gradient(180deg,rgba(126,242,215,0.88),rgba(152,181,255,0.38))]"
                          style={{ height: `${height}%` }}
                        />
                      ))}
                    </div>
                ...
                <div className="mt-4 grid gap-4 sm:grid-cols-3">
                  {["Signal fidelity", "Multi-agent control", "Launch telemetry"].map(
                    (label, index) => (
                      <div
                        key={label}
                        className="rounded-[1.05rem] border border-white/[0.08] bg-white/[0.03] p-4"
                      >
          ...
          <div className="hero-float glass-panel absolute -left-5 top-[16%] rounded-[1.4rem] px-4 py-3">
            <p className="text-[0.65rem] uppercase tracking-[0.22em] text-white/[0.42]">mode</p>
            <p className="mt-2 font-display text-xl text-white">Launch Focus</p>
          </div>
          ...
          <div className="hero-float glass-panel absolute -right-4 bottom-[16%] rounded-[1.4rem] px-4 py-3">
            ...
            <p className="mt-2 font-display text-xl text-white">98.4%</p>
          </div>
        </div>
      </div>
    </section>
